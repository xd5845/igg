...
?>
						...
						<br />
                      		<div class="group-control">
							<label class="control-label"><a onclick="javascript:autofill(11);" href="#"><?php _e('Sileo预览页'); ?></a></label>
							<div class="controls">
<?php
		$repourl = base64_decode(DCRM_REPOURL);
		$repourl = substr($repourl, -1) == '/' ? $repourl : $repourl.'/';
		$rewrite_mod = get_option('rewrite_mod');
		if ($rewrite_mod == 3)
			$depiction_url1 = $repourl . 'sileo/' . $request_id;
		else
			$depiction_url1 = $repourl . 'sileo/' . $request_id;
?>
								<input id="urlinput" type="text" style="width: 400px;" name="SileoDepiction" value="<?php echo($depiction_url1); ?>"/>
								<p class="help-block"><?php printf(__('Default Depiction: %s'), $depiction_url1); ?></p>
								<p class="help-block"><a class="btn btn-warning" href="<?php echo($depiction_url1); ?>" target="_blank"><?php _e('Preview Default Depiction'); ?></a></p>
							</div>
						</div>
...
?>
	...
	function autofill(opt) {
		if (opt == 1) {
			...
			var pstr = document.getElementsByName("SileoDepiction")[0].value;
			if (pstr.length != 0) {
				if (pstr.indexOf("https://") == -1) {
					document.getElementsByName("SileoDepiction")[0].value = "https://" + pstr;
				} else {
					document.getElementsByName("SileoDepiction")[0].value = "";
				}
			} else {
				document.getElementsByName("SileoDepiction")[0].value = "<?php echo($depiction_url1); ?>";
			} 
		} else if (opt == 9) {
